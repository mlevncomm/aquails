...
import { EARN_RULES, convertPointsToCoupon } from '@/services/loyaltyService';
import { useToastStore } from '@/components/Toast';

export default function CustomerLoyaltyPage() {
  const { data, refresh } = useLoyaltyStore();
  ...
  return (
    <div className="p-4 md:p-6">
      <h1 className="text-xl md:text-2xl font-bold text-[#0D2137] mb-1">Puanlarim</h1>
      <p className="text-sm text-[#8B9DAF] mb-6">Aquails sadakat programina katilin, alisverislerinizden puan kazanin.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white border border-[#E8F0FE] rounded-2xl p-6">
          ...
          <p className="text-3xl font-bold text-[#0D2137]">{data.totalEarned}</p>
          <p className="text-xs text-[#8B9DAF] mt-1">Toplam Kazanilan</p>
        </div>
        <div className="bg-white border border-[#E8F0FE] rounded-2xl p-6">
          <Gift className="w-8 h-8 text-[#00C9A7] mb-3" />
          <p className="text-3xl font-bold text-[#0D2137]">{data.totalRedeemed}</p>
          <p className="text-xs text-[#8B9DAF] mt-1">Kullanilan</p>
        </div>
      </div>

      {/* Convert to Coupon */}
      <div className="bg-white border border-[#E8F0FE] rounded-2xl p-5 mb-6">
        <h3 className="text-sm font-semibold text-[#0D2137] mb-3">Puani Kupona Cevir</h3>
        ...
        <div className="space-y-2">
          {EARN_RULES.map(rule => (
            <div key={rule.action} className="flex items-center justify-between p-3 bg-[#F8FBFF] rounded-xl">
              ...
              <span className="text-sm font-semibold text-[#1A73E8]">+{rule.points} puan</span>
            </div>
          ))}
        </div>
      </div>
