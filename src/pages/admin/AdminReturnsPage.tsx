    ...
    <>
      ...
      <div className="bg-white border border-[#E8F0FE] rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-sm text-[#8B9DAF]">Henüz iade/değişim talebi yok.</div>
        ) : (
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-[#F0F6FF] last:border-0 hover:bg-[#F8FBFF]/50">
                    ...
                    <td className="px-4 py-3 text-sm text-[#1A73E8]">{item.orderNumber}</td>
                    <td className="px-4 py-3 text-sm text-[#5A6B7B]">{item.productName}</td>
                    <td className="px-4 py-3 text-sm">{item.type === 'return' ? 'İade' : 'Değişim'}</td>
                    <td className="px-4 py-3 text-sm text-[#5A6B7B] max-w-[200px] truncate">{item.reason}</td>
