...
export default function AdminCustomersPage() {
  ...
  useEffect(() => {
    ...
    getAdminCustomers()
      ...
      .catch((err) => { if (!cancelled) setError(err instanceof Error ? err.message : 'Müşteriler yüklenemedi.'); })
      ...
      <div className="bg-white border border-[#E8F0FE] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-[#F8FBFF]">{['Müşteri', 'E-posta', 'Telefon', 'Sipariş', 'Kayıt', 'İşlem'].map(h => <th key={h} className="text-left px-4 py-3 text-[11px] font-semibold text-[#8B9DAF] uppercase whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} className="border-b border-[#F0F6FF] last:border-0 hover:bg-[#F8FBFF]/50">
                  ...
                  <td className="px-4 py-3 text-sm text-[#0D2137]">{c.orderCount}</td>
                  <td className="px-4 py-3 text-[13px] text-[#8B9DAF] whitespace-nowrap">{formatDate(c.registeredAt)}</td>
        ...
        {filtered.length === 0 && <div className="text-center py-8 text-sm text-[#8B9DAF]">Müşteri bulunamadı</div>}
      </div>
      </>
  );
}
