...
function buildTimeline(status: string, date: string) {
  const idx = STATUS_ORDER.indexOf(status);
  return TIMELINE_STEPS.map((t, i) => ({
    ...t,
    done: status === 'cancelled' || status === 'returned' ? i === 0 : i <= idx,
    date: i === 0 ? date : i <= idx ? date : '',
    desc: t.step,
  }));
}
...
export default function AdminOrderDetailPage() {
  ...
  const timeline = buildTimeline(currentStatus, order.date);
    ...
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <AdminCard>
            <h3 className="text-sm font-semibold text-slate-800 mb-4 flex items-center gap-2">
              <MapPin className="w-4 h-4 text-sky-600" /> Teslimat
            </h3>
            <div className="bg-slate-50 rounded-xl p-4">
