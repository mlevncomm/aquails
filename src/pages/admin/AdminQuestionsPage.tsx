...
export default function AdminQuestionsPage() {
  ...
  const refresh = () => setItems(getQuestions());
  ...
  const submitAnswer = (id: string) => {
    ...
    answerQuestion(id, answerForm.text.trim());
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#8B9DAF]">{q.productName} | {formatDate(q.createdAt)}</p>
                  </div>
                </div>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${q.status === 'answered' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
              ...
              {q.status === 'pending' && (
                <div className="ml-10 flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 text-sm border border-[#D6E3F0] rounded-xl bg-[#F8FBFF] focus:outline-none focus:border-[#1A73E8]"
                  />
                  <button onClick={() => submitAnswer(q.id)} className="bg-[#1A73E8] text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-[#1557B0]">
                    <Check className="w-4 h-4" />
                  </button>
                </div>
              )}
              <div className="flex justify-end mt-3">
                <button onClick={() => remove(q.id)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-[#8B9DAF] hover:text-red-500">
