        ...
        <AnimatePresence mode="wait">
          {currentStep < steps.length ? (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <h2 className="text-xl font-semibold text-aq-text mb-6">{steps[currentStep].title}</h2>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition-all ${
                        isSelected
                          ...
                          : 'border-aq-border/60 bg-white hover:border-aq-border/60'
                      }`}
                    >
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isSelected ? 'bg-aq-aqua' : 'bg-aq-sky'}`}>
                        <Icon className={`w-5 h-5 ${isSelected ? 'text-aq-text' : 'text-aq-blue'}`} />
                      </div>
                      <div>
                        <p className={`text-sm font-semibold ${isSelected ? 'text-aq-blue' : 'text-aq-text'}`}>{opt.label}</p>
              ...
              <div className="flex items-center justify-between mt-8">
                ...
                <button
                  onClick={handleNext}
                  disabled={!isAnswered}
                  className="flex items-center gap-2 bg-aq-blue text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-aq-deep hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            >
              <div className="text-center mb-8">
                ...
                <p className="text-sm text-aq-muted mt-2">İhtiyaclarınıza en uygun {recommendations?.length || 0} ürün bulduk.</p>
              </div>
                ...
                <ScrollReveal key={rec.product.id} delay={i * 0.1}>
                  <div className="bg-white border border-aq-border/60 rounded-2xl p-5 mb-4 hover:shadow-sm transition-all">
                    <div className="flex items-start gap-4">
                      <div className="w-20 h-20 bg-aq-sky rounded-xl overflow-hidden flex-shrink-0">
                        <img
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-medium text-aq-blue bg-aq-sky px-2 py-0.5 rounded-full">%{rec.score} Uyum</span>
                          {rec.tags.map(t => (
                            <span key={t} className="text-[10px] text-aq-muted bg-aq-ice px-2 py-0.5 rounded-full">{t}</span>
                          ))}
                        </div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-lg font-semibold text-aq-blue">{rec.product.price.toLocaleString('tr-TR')} ₺</span>
                          {rec.product.oldPrice && (
                            <span className="text-xs text-aq-muted line-through">{rec.product.oldPrice.toLocaleString('tr-TR')} ₺</span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-2 mt-3">
                          <Link to={`/urun/${rec.product.slug}`} className="flex items-center gap-1.5 bg-aq-blue text-white text-xs font-semibold px-4 py-2 rounded-xl hover:bg-aq-deep hover:text-white transition-all">
              ...
              <div className="flex justify-center gap-3 mt-8">
                <button onClick={handleRestart} className="flex items-center gap-2 text-sm font-medium text-aq-muted hover:text-aq-text transition-all">
                  <ArrowLeft className="w-4 h-4" /> Bastan Basla
                </button>
                <Link to="/urunler" className="flex items-center gap-2 bg-aq-blue text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-aq-deep hover:text-white transition-all">
